...
class ReplayMemory:
	def __init__(self, mem_size, s_dims):
		self.mem_size = mem_size

		self.s = np.zeros((mem_size, *s_dims))
		self.a = np.zeros(mem_size)
		self.r = np.zeros(mem_size)
		self.s_ = np.zeros((mem_size, *s_dims))
...
class ReplayBuffer(object):
    def __init__(self, size, frame_history_len, flat=False):
    ...
    def _encode_observation(self, idx):
        """If you are using a latent space"""
        end_idx = idx + 1  # make noninclusive
        start_idx = end_idx - self.frame_history_len
        # this checks if we are using low-dimensional observations, such as RAM
        # state, in which case we just directly return the latest RAM.
        """COMMENTING THIS OUT FOR STACKED_VAE"""
